...
2. **3 MCP Services** - Each handling specific governance concerns
...
User Prompt → Orchestrator → LLM Analysis
    ...
    1. Identity MCP (Authorize)
                ↓
    2. Operations MCP (Execute)
                ↓
    3. Audit MCP (Log Everything)
                ↓
...
| `"Restart the service"` | Authorization → Execute → Audit |
...
```json
{
  ...
  "logs": [
    ...
    },
    {
      ...
      "user": {"user_id": "alice", "role": "admin"},
      "action": "restart_service",
      "result": {"success": true, "message": "Service restarted successfully"}
    },
    {
      "tool": "log_audit",
      "user_id": "alice",
...
3. **Test System Actions**
   - "Restart the service"
   - Observe: Authorization → Execute → Audit flow
...
#### Scenario 1: Authorized Action (Admin)
```
User: alice (admin)
Prompt: "restart the service"
Expected: Authorization granted → Execute action → Audit logged
```

#### Scenario 2: Authorized Action (Operator)
```
User: bob (operator)
Prompt: "restart the service"
Expected: Authorization granted → Execute action → Audit logged
