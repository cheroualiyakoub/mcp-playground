...
def evaluate_risk_tool(tool_input: str) -> str:
    """Evaluate risk level of an action"""
    try:
        data = json.loads(tool_input)
    except json.JSONDecodeError as e:
        return json.dumps({"error": f"Invalid JSON input: {str(e)}", "risk_score": 100})
    
    risk_url = os.getenv("RISK_MCP_URL", "http://risk:8002")
    resp = _post(risk_url, "/evaluate", data)
    entry = {"tool": "evaluate_risk", "user": data.get("user"), "action": data.get("action"), "result": resp}
    _record(entry)
    return json.dumps(resp)


def approve_tool(tool_input: str) -> str:
    """Approve a high-risk action"""
    try:
        data = json.loads(tool_input)
    except json.JSONDecodeError as e:
        return json.dumps({"error": f"Invalid JSON input: {str(e)}", "approved": False})
    
    risk_url = os.getenv("RISK_MCP_URL", "http://risk:8002")
    resp = _post(risk_url, "/approve", data)
    entry = {"tool": "approve", "user": data.get("user"), "action": data.get("action"), "result": resp}
    _record(entry)
    return json.dumps(resp)


def query_observability_tool(tool_input: str) -> str:
    """Query system metrics and observability data"""
    try:
        data = json.loads(tool_input)
    except json.JSONDecodeError as e:
        return json.dumps({"error": f"Invalid JSON input: {str(e)}", "metrics": {}})
    
    obs_url = os.getenv("OBSERVABILITY_MCP_URL", "http://observability:8004")
    resp = _post(obs_url, "/query", data)
    entry = {"tool": "query_observability", "query_type": data.get("query_type"), "user_id": data.get("user_id"), "result": resp}
    _record(entry)
    return json.dumps(resp)


def check_secret_tool(tool_input: str) -> str:
    """Check access to a secret"""
    try:
        data = json.loads(tool_input)
    except json.JSONDecodeError as e:
        return json.dumps({"error": f"Invalid JSON input: {str(e)}", "access": False})
    
    sec_url = os.getenv("SECRETS_MCP_URL", "http://secrets:8005")
    resp = _post(sec_url, "/check_secret", data)
    entry = {"tool": "check_secret", "path": data.get("path"), "user": data.get("user"), "result": resp}
    _record(entry)
    return json.dumps(resp)
...
def get_langchain_tools():
    ...
    return [
        ...
        Tool(
            name="evaluate_risk",
            func=evaluate_risk_tool,
            description='Evaluate risk level of an action. Input: JSON string with user dict, action string, and trace_id'
        ),
        Tool(
            name="approve",
            func=approve_tool,
            description='Approve a high-risk action. Input: JSON string with user dict, action string, and trace_id'
        ),
        Tool(
            name="query_observability",
            func=query_observability_tool,
            description='Query system metrics and observability data. Input: JSON string with query_type and user_id'
        ),
        Tool(
            name="check_secret",
            func=check_secret_tool,
            description='Check access to a secret. Input: JSON string with path string and user dict'
        ),
        Tool(
            name="log_audit",
            func=log_audit_tool,
